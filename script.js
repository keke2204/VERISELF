...
function initApiBridge() {
  const input = adv('apiBaseUrl');
  const save = adv('saveApiBtn');
  const test = adv('testApiBtn');
  const syncSource = adv('syncSourceBtn');
  ...
  save?.addEventListener('click', () => {
    ...
    if (value) {
      ...
      localStorage.removeItem('veriself-api-base');
  ...
  syncSource?.addEventListener('click', async () => {
    if (!sourceFile) return alert('Upload a source image first.');
    try {
      setApiStatus('UPLOADING…');
      const form = new FormData();
      form.append('file', sourceFile, sourceFile.name);
      const data = await apiRequest('/api/v1/media/register', { method: 'POST', body: form });
      localStorage.setItem('veriself-media-id', String(data.id));
      adv('apiMediaId').textContent = data.id;
      setApiStatus('SOURCE REGISTERED');
    } catch (err) {
      console.error('Backend source registration failed:', err);
      setApiStatus('ERROR');
      alert(err.message);
    }
  });

  syncCandidate?.addEventListener('click', async () => {
    if (!sourceFile) return alert('Upload a source image first.');
    const mediaId = localStorage.getItem('veriself-media-id');
    const input = adv('candidateInput');
    const candidate = input?.files?.[0];
    if (!mediaId || !candidate) return alert('Register the source and choose a candidate image first.');
    try {
      setApiStatus('COMPARING…');
      const form = new FormData();
      form.append('source_id', mediaId);
      form.append('candidate', candidate, candidate.name);
      const data = await apiRequest('/api/v1/media/compare', { method: 'POST', body: form });
      adv('apiMediaId').textContent = String(data.source_id);
      adv('advDistance').textContent = data.hamming_distance + ' BITS / 64';
      adv('advSimilarity').textContent = data.similarity_percent + '%';
      adv('advVerdict').textContent = data.result;
      adv('candidateResult').style.display = 'block';
      setApiStatus('BACKEND MATCH COMPLETE');
    } catch (err) {
      console.error('Backend candidate comparison failed:', err);
      setApiStatus('ERROR');
      alert(err.message);
    }
  });
...
document.addEventListener('DOMContentLoaded', initApiBridge);
